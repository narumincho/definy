...
impl From<UAutocapitalize> for String {
    fn from(value: UAutocapitalize) -> Self {
...
impl From<UDir> for String {
    fn from(value: UDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<UPopover> for String {
    fn from(value: UPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-u-element
