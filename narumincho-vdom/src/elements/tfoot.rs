...
impl From<TfootAutocapitalize> for String {
    fn from(value: TfootAutocapitalize) -> Self {
...
impl From<TfootDir> for String {
    fn from(value: TfootDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum TfootPopover {
    Auto,
    Manual,
}
...
impl From<TfootPopover> for String {
    fn from(value: TfootPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/tables.html#the-tfoot-element
...
impl From<Tfoot> for crate::Node {
    fn from(value: Tfoot) -> Self {
        value.into_node()
    }
}
