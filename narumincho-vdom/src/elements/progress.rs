...
pub enum ProgressAutocapitalize {
    Off,
    None,
    Characters,
    Words,
...
impl From<ProgressAutocapitalize> for String {
    fn from(value: ProgressAutocapitalize) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ProgressDir {
    Ltr,
    Rtl,
    Auto,
}
...
impl From<ProgressDir> for String {
    fn from(value: ProgressDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ProgressPopover {
    Auto,
    Manual,
}

impl ProgressPopover {
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::Auto => "auto",
            Self::Manual => "manual",
...
impl From<ProgressPopover> for String {
    fn from(value: ProgressPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/form-elements.html#the-progress-element
...
impl Progress {
    ...
    pub fn into_node(self) -> crate::Node {
        crate::Node::Element(crate::Element {
            element_name: "progress".to_string(),
...
impl From<Progress> for crate::Node {
