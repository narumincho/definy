...
pub enum ArticleAutocapitalize {
    Off,
    None,
    Characters,
    Words,
...
impl From<ArticleAutocapitalize> for String {
    fn from(value: ArticleAutocapitalize) -> Self {
...
impl From<ArticleDir> for String {
    fn from(value: ArticleDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ArticlePopover {
    Auto,
    Manual,
}
...
impl From<ArticlePopover> for String {
    fn from(value: ArticlePopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-article-element
...
impl Article {
    ...
    pub fn into_node(self) -> crate::Node {
        crate::Node::Element(crate::Element {
            element_name: "article".to_string(),
...
impl From<Article> for crate::Node {
