...
impl From<H3Autocapitalize> for String {
    fn from(value: H3Autocapitalize) -> Self {
...
impl From<H3Dir> for String {
    fn from(value: H3Dir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<H3Popover> for String {
    fn from(value: H3Popover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-h3-element
...
impl From<H3> for crate::Node {
    fn from(value: H3) -> Self {
        value.into_node()
    }
}
