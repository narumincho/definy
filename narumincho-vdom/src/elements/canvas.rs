...
impl From<CanvasAutocapitalize> for String {
    fn from(value: CanvasAutocapitalize) -> Self {
...
impl From<CanvasDir> for String {
    fn from(value: CanvasDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CanvasPopover {
    Auto,
    Manual,
}
...
impl From<CanvasPopover> for String {
    fn from(value: CanvasPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/canvas.html#canvas
...
impl From<Canvas> for crate::Node {
