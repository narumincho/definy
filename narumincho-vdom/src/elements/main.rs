...
impl From<MainAutocapitalize> for String {
    fn from(value: MainAutocapitalize) -> Self {
...
impl From<MainDir> for String {
    fn from(value: MainDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MainPopover {
    Auto,
    Manual,
}
...
impl From<MainPopover> for String {
    fn from(value: MainPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/grouping-content.html#the-main-element
