...
impl From<MenuAutocapitalize> for String {
    fn from(value: MenuAutocapitalize) -> Self {
...
impl From<MenuDir> for String {
    fn from(value: MenuDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MenuPopover {
    Auto,
    Manual,
}
...
impl From<MenuPopover> for String {
    fn from(value: MenuPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/grouping-content.html#menus
