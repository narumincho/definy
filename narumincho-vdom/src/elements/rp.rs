...
impl From<RpAutocapitalize> for String {
    fn from(value: RpAutocapitalize) -> Self {
...
impl From<RpDir> for String {
    fn from(value: RpDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<RpPopover> for String {
    fn from(value: RpPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-rp-element
