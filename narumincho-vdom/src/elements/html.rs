...
impl From<HtmlAutocapitalize> for String {
    fn from(value: HtmlAutocapitalize) -> Self {
...
impl From<HtmlDir> for String {
    fn from(value: HtmlDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HtmlPopover {
    Auto,
    Manual,
}
...
impl From<HtmlPopover> for String {
    fn from(value: HtmlPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/semantics.html#the-html-element
