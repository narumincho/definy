...
impl From<WbrAutocapitalize> for String {
    fn from(value: WbrAutocapitalize) -> Self {
...
impl From<WbrDir> for String {
    fn from(value: WbrDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<WbrPopover> for String {
    fn from(value: WbrPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-wbr-element
...
impl From<Wbr> for crate::Node {
    fn from(value: Wbr) -> Self {
        value.into_node()
    }
}
