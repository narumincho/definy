...
impl From<RtAutocapitalize> for String {
    fn from(value: RtAutocapitalize) -> Self {
...
impl From<RtDir> for String {
    fn from(value: RtDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<RtPopover> for String {
    fn from(value: RtPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-rt-element
