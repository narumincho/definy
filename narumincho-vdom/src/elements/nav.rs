...
impl From<NavAutocapitalize> for String {
    fn from(value: NavAutocapitalize) -> Self {
...
impl From<NavDir> for String {
    fn from(value: NavDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<NavPopover> for String {
    fn from(value: NavPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-nav-element
...
impl From<Nav> for crate::Node {
    fn from(value: Nav) -> Self {
        value.into_node()
    }
}
