...
impl From<HeadAutocapitalize> for String {
    fn from(value: HeadAutocapitalize) -> Self {
...
impl From<HeadDir> for String {
    fn from(value: HeadDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HeadPopover {
    Auto,
    Manual,
}
...
impl From<HeadPopover> for String {
    fn from(value: HeadPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/semantics.html#the-head-element
