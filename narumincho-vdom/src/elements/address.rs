...
pub enum AddressAutocapitalize {
    Off,
    None,
    Characters,
    Words,
...
impl From<AddressAutocapitalize> for String {
    fn from(value: AddressAutocapitalize) -> Self {
...
impl From<AddressDir> for String {
    fn from(value: AddressDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AddressPopover {
    Auto,
    Manual,
}
...
impl From<AddressPopover> for String {
    fn from(value: AddressPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-address-element
...
impl From<Address> for crate::Node {
