...
impl From<FooterAutocapitalize> for String {
    fn from(value: FooterAutocapitalize) -> Self {
...
impl From<FooterDir> for String {
    fn from(value: FooterDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FooterPopover {
    Auto,
    Manual,
}
...
impl From<FooterPopover> for String {
    fn from(value: FooterPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-footer-element
...
impl From<Footer> for crate::Node {
