...
impl From<DdAutocapitalize> for String {
    fn from(value: DdAutocapitalize) -> Self {
...
impl From<DdDir> for String {
    fn from(value: DdDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<DdPopover> for String {
    fn from(value: DdPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/grouping-content.html#the-dd-element
...
impl From<Dd> for crate::Node {
    fn from(value: Dd) -> Self {
        value.into_node()
    }
}
