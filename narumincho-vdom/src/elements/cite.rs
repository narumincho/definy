...
impl From<CiteAutocapitalize> for String {
    fn from(value: CiteAutocapitalize) -> Self {
...
impl From<CiteDir> for String {
    fn from(value: CiteDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CitePopover {
    Auto,
    Manual,
}
...
impl From<CitePopover> for String {
    fn from(value: CitePopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-cite-element
