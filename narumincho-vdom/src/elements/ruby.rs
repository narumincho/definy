...
impl From<RubyAutocapitalize> for String {
    fn from(value: RubyAutocapitalize) -> Self {
...
impl From<RubyDir> for String {
    fn from(value: RubyDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RubyPopover {
    Auto,
    Manual,
}
...
impl From<RubyPopover> for String {
    fn from(value: RubyPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-ruby-element
...
impl From<Ruby> for crate::Node {
    fn from(value: Ruby) -> Self {
        value.into_node()
    }
}
