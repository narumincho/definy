...
impl From<CodeAutocapitalize> for String {
    fn from(value: CodeAutocapitalize) -> Self {
...
impl From<CodeDir> for String {
    fn from(value: CodeDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CodePopover {
    Auto,
    Manual,
}
...
impl From<CodePopover> for String {
    fn from(value: CodePopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-code-element
