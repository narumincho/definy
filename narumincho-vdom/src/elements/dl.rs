...
impl From<DlAutocapitalize> for String {
    fn from(value: DlAutocapitalize) -> Self {
...
impl From<DlDir> for String {
    fn from(value: DlDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<DlPopover> for String {
    fn from(value: DlPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/grouping-content.html#the-dl-element
...
impl From<Dl> for crate::Node {
    fn from(value: Dl) -> Self {
        value.into_node()
    }
}
