...
impl From<AsideAutocapitalize> for String {
    fn from(value: AsideAutocapitalize) -> Self {
...
impl From<AsideDir> for String {
    fn from(value: AsideDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AsidePopover {
    Auto,
    Manual,
}
...
impl From<AsidePopover> for String {
    fn from(value: AsidePopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-aside-element
...
impl From<Aside> for crate::Node {
