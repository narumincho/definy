...
impl From<BdoAutocapitalize> for String {
    fn from(value: BdoAutocapitalize) -> Self {
...
impl From<BdoDir> for String {
    fn from(value: BdoDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<BdoPopover> for String {
    fn from(value: BdoPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-bdo-element
...
impl From<Bdo> for crate::Node {
    fn from(value: Bdo) -> Self {
        value.into_node()
    }
}
