...
impl From<SearchAutocapitalize> for String {
    fn from(value: SearchAutocapitalize) -> Self {
...
impl From<SearchDir> for String {
    fn from(value: SearchDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SearchPopover {
    Auto,
    Manual,
}
...
impl From<SearchPopover> for String {
    fn from(value: SearchPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/grouping-content.html#the-search-element
...
impl From<Search> for crate::Node {
