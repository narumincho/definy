...
impl From<HgroupAutocapitalize> for String {
    fn from(value: HgroupAutocapitalize) -> Self {
...
impl From<HgroupDir> for String {
    fn from(value: HgroupDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HgroupPopover {
    Auto,
    Manual,
}
...
impl From<HgroupPopover> for String {
    fn from(value: HgroupPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-hgroup-element
...
impl From<Hgroup> for crate::Node {
    fn from(value: Hgroup) -> Self {
        value.into_node()
    }
}
