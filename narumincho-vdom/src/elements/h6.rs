...
impl From<H6Autocapitalize> for String {
    fn from(value: H6Autocapitalize) -> Self {
...
impl From<H6Dir> for String {
    fn from(value: H6Dir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<H6Popover> for String {
    fn from(value: H6Popover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-h6-element
...
impl From<H6> for crate::Node {
    fn from(value: H6) -> Self {
        value.into_node()
    }
}
