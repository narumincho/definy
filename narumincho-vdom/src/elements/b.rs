...
impl From<BAutocapitalize> for String {
    fn from(value: BAutocapitalize) -> Self {
...
impl From<BDir> for String {
    fn from(value: BDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<BPopover> for String {
    fn from(value: BPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-b-element
