...
impl From<IAutocapitalize> for String {
    fn from(value: IAutocapitalize) -> Self {
...
impl From<IDir> for String {
    fn from(value: IDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<IPopover> for String {
    fn from(value: IPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-i-element
