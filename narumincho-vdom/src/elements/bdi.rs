...
impl From<BdiAutocapitalize> for String {
    fn from(value: BdiAutocapitalize) -> Self {
...
impl From<BdiDir> for String {
    fn from(value: BdiDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<BdiPopover> for String {
    fn from(value: BdiPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-bdi-element
...
impl From<Bdi> for crate::Node {
    fn from(value: Bdi) -> Self {
        value.into_node()
    }
}
