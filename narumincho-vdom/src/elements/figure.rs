...
impl From<FigureAutocapitalize> for String {
    fn from(value: FigureAutocapitalize) -> Self {
...
impl From<FigureDir> for String {
    fn from(value: FigureDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FigurePopover {
    Auto,
    Manual,
}
...
impl From<FigurePopover> for String {
    fn from(value: FigurePopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/grouping-content.html#the-figure-element
...
impl From<Figure> for crate::Node {
