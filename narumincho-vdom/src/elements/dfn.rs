...
impl From<DfnAutocapitalize> for String {
    fn from(value: DfnAutocapitalize) -> Self {
...
impl From<DfnDir> for String {
    fn from(value: DfnDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<DfnPopover> for String {
    fn from(value: DfnPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-dfn-element
...
impl From<Dfn> for crate::Node {
    fn from(value: Dfn) -> Self {
        value.into_node()
    }
}
