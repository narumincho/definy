...
impl From<SampAutocapitalize> for String {
    fn from(value: SampAutocapitalize) -> Self {
...
impl From<SampDir> for String {
    fn from(value: SampDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SampPopover {
    Auto,
    Manual,
}
...
impl From<SampPopover> for String {
    fn from(value: SampPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-samp-element
...
impl From<Samp> for crate::Node {
    fn from(value: Samp) -> Self {
        value.into_node()
    }
}
