...
impl From<H2Autocapitalize> for String {
    fn from(value: H2Autocapitalize) -> Self {
...
impl From<H2Dir> for String {
    fn from(value: H2Dir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<H2Popover> for String {
    fn from(value: H2Popover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-h2-element
...
impl From<H2> for crate::Node {
    fn from(value: H2) -> Self {
        value.into_node()
    }
}
