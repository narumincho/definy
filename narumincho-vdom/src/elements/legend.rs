...
impl From<LegendAutocapitalize> for String {
    fn from(value: LegendAutocapitalize) -> Self {
...
impl From<LegendDir> for String {
    fn from(value: LegendDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LegendPopover {
    Auto,
    Manual,
}
...
impl From<LegendPopover> for String {
    fn from(value: LegendPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/form-elements.html#the-legend-element
...
impl From<Legend> for crate::Node {
