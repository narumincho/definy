...
impl From<MarkAutocapitalize> for String {
    fn from(value: MarkAutocapitalize) -> Self {
...
impl From<MarkDir> for String {
    fn from(value: MarkDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MarkPopover {
    Auto,
    Manual,
}
...
impl From<MarkPopover> for String {
    fn from(value: MarkPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-mark-element
