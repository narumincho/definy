...
impl From<LabelAutocapitalize> for String {
    fn from(value: LabelAutocapitalize) -> Self {
...
impl From<LabelDir> for String {
    fn from(value: LabelDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LabelPopover {
    Auto,
    Manual,
}
...
impl From<LabelPopover> for String {
    fn from(value: LabelPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/forms.html#the-label-element
...
impl From<Label> for crate::Node {
