...
impl From<H5Autocapitalize> for String {
    fn from(value: H5Autocapitalize) -> Self {
...
impl From<H5Dir> for String {
    fn from(value: H5Dir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<H5Popover> for String {
    fn from(value: H5Popover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-h5-element
...
impl From<H5> for crate::Node {
    fn from(value: H5) -> Self {
        value.into_node()
    }
}
