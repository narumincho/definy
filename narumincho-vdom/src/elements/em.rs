...
impl From<EmAutocapitalize> for String {
    fn from(value: EmAutocapitalize) -> Self {
...
impl From<EmDir> for String {
    fn from(value: EmDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<EmPopover> for String {
    fn from(value: EmPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-em-element
