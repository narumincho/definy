...
pub enum OptionElementAutocapitalize {
    Off,
    None,
    Characters,
    Words,
...
impl OptionElementAutocapitalize {
...
impl From<OptionElementAutocapitalize> for String {
    fn from(value: OptionElementAutocapitalize) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OptionElementDir {
    Ltr,
    Rtl,
    Auto,
}

impl OptionElementDir {
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::Ltr => "ltr",
            Self::Rtl => "rtl",
...
impl From<OptionElementDir> for String {
    fn from(value: OptionElementDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OptionElementPopover {
    Auto,
    Manual,
}

impl OptionElementPopover {
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::Auto => "auto",
            Self::Manual => "manual",
...
impl From<OptionElementPopover> for String {
    fn from(value: OptionElementPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/form-elements.html#the-option-element
...
impl Default for OptionElement {
    fn default() -> Self {
        Self::new()
    }
}
...
impl OptionElement {
    ...
    pub fn default_selected(mut self, value: bool) -> Self {
        self.attributes.retain(|(key, _)| key != "defaultSelected");
        if value {
            self.attributes
                .push(("defaultSelected".to_string(), String::new()));
        }
        self
    }
    ...
    pub fn selected(mut self, value: bool) -> Self {
        self.attributes.retain(|(key, _)| key != "selected");
        if value {
            self.attributes
...
impl From<OptionElement> for crate::Node {
