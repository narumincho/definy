...
impl From<SlotAutocapitalize> for String {
    fn from(value: SlotAutocapitalize) -> Self {
...
impl From<SlotDir> for String {
    fn from(value: SlotDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SlotPopover {
    Auto,
    Manual,
}
...
impl From<SlotPopover> for String {
    fn from(value: SlotPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/scripting.html#the-slot-element
