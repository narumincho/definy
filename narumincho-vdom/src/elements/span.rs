...
impl From<SpanAutocapitalize> for String {
    fn from(value: SpanAutocapitalize) -> Self {
...
impl From<SpanDir> for String {
    fn from(value: SpanDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SpanPopover {
    Auto,
    Manual,
}
...
impl From<SpanPopover> for String {
    fn from(value: SpanPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-span-element
