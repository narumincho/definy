...
impl From<StrongAutocapitalize> for String {
    fn from(value: StrongAutocapitalize) -> Self {
...
impl From<StrongDir> for String {
    fn from(value: StrongDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum StrongPopover {
    Auto,
    Manual,
}
...
impl From<StrongPopover> for String {
    fn from(value: StrongPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-strong-element
...
impl From<Strong> for crate::Node {
