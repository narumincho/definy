...
impl From<VarAutocapitalize> for String {
    fn from(value: VarAutocapitalize) -> Self {
...
impl From<VarDir> for String {
    fn from(value: VarDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<VarPopover> for String {
    fn from(value: VarPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-var-element
