...
pub enum CaptionAutocapitalize {
    Off,
    None,
    Characters,
    Words,
...
impl From<CaptionAutocapitalize> for String {
    fn from(value: CaptionAutocapitalize) -> Self {
...
impl From<CaptionDir> for String {
    fn from(value: CaptionDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CaptionPopover {
    Auto,
    Manual,
}
...
impl From<CaptionPopover> for String {
    fn from(value: CaptionPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/tables.html#the-caption-element
...
impl Caption {
    ...
    pub fn into_node(self) -> crate::Node {
        crate::Node::Element(crate::Element {
            element_name: "caption".to_string(),
...
impl From<Caption> for crate::Node {
