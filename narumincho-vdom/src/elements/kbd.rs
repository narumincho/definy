...
impl From<KbdAutocapitalize> for String {
    fn from(value: KbdAutocapitalize) -> Self {
...
impl From<KbdDir> for String {
    fn from(value: KbdDir) -> Self {
        value.as_str().to_string()
    }
}
...
impl From<KbdPopover> for String {
    fn from(value: KbdPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-kbd-element
