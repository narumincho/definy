...
impl From<HeaderAutocapitalize> for String {
    fn from(value: HeaderAutocapitalize) -> Self {
...
impl From<HeaderDir> for String {
    fn from(value: HeaderDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HeaderPopover {
    Auto,
    Manual,
}
...
impl From<HeaderPopover> for String {
    fn from(value: HeaderPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/sections.html#the-header-element
...
impl From<Header> for crate::Node {
