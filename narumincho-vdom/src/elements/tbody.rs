...
impl From<TbodyAutocapitalize> for String {
    fn from(value: TbodyAutocapitalize) -> Self {
...
impl From<TbodyDir> for String {
    fn from(value: TbodyDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum TbodyPopover {
    Auto,
    Manual,
}
...
impl From<TbodyPopover> for String {
    fn from(value: TbodyPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/tables.html#the-tbody-element
...
impl From<Tbody> for crate::Node {
    fn from(value: Tbody) -> Self {
        value.into_node()
    }
}
