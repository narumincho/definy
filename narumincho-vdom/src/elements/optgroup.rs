...
pub enum OptgroupAutocapitalize {
    Off,
    None,
    Characters,
    Words,
...
impl From<OptgroupAutocapitalize> for String {
    fn from(value: OptgroupAutocapitalize) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OptgroupDir {
    Ltr,
    Rtl,
    Auto,
}
...
impl From<OptgroupDir> for String {
    fn from(value: OptgroupDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OptgroupPopover {
    Auto,
    Manual,
}

impl OptgroupPopover {
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::Auto => "auto",
            Self::Manual => "manual",
...
impl From<OptgroupPopover> for String {
    fn from(value: OptgroupPopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/form-elements.html#the-optgroup-element
...
impl Optgroup {
    ...
    pub fn into_node(self) -> crate::Node {
        crate::Node::Element(crate::Element {
            element_name: "optgroup".to_string(),
...
impl From<Optgroup> for crate::Node {
    fn from(value: Optgroup) -> Self {
        value.into_node()
    }
}
