...
pub enum FencedframeAutocapitalize {
    Off,
    None,
    Characters,
    Words,
...
impl FencedframeAutocapitalize {
...
impl From<FencedframeAutocapitalize> for String {
    fn from(value: FencedframeAutocapitalize) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FencedframeDir {
    Ltr,
    Rtl,
    Auto,
}
...
impl From<FencedframeDir> for String {
    fn from(value: FencedframeDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FencedframePopover {
    Auto,
    Manual,
}

impl FencedframePopover {
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::Auto => "auto",
            Self::Manual => "manual",
...
impl From<FencedframePopover> for String {
    fn from(value: FencedframePopover) -> Self {
        value.as_str().to_string()
    }
}
...
impl Fencedframe {
    ...
    pub fn into_node(self) -> crate::Node {
        crate::Node::Element(crate::Element {
            element_name: "fencedframe".to_string(),
...
impl From<Fencedframe> for crate::Node {
    fn from(value: Fencedframe) -> Self {
        value.into_node()
    }
}
