...
impl From<TimeAutocapitalize> for String {
    fn from(value: TimeAutocapitalize) -> Self {
...
impl From<TimeDir> for String {
    fn from(value: TimeDir) -> Self {
        value.as_str().to_string()
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum TimePopover {
    Auto,
    Manual,
}
...
impl From<TimePopover> for String {
    fn from(value: TimePopover) -> Self {
        value.as_str().to_string()
    }
}

/// HTML Content Attributes for https://html.spec.whatwg.org/multipage/text-level-semantics.html#the-time-element
...
impl Time {
    ...
    pub fn date_time(self, value: impl Into<String>) -> Self {
