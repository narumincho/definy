...
fn render_node(
    ...
    let container_style = if is_multiline {
        "display: flex; flex-direction: column; align-items: flex-start; gap: 0.25rem; margin: 0.15rem 0;"
    } else {
        "display: inline-flex; align-items: center; gap: 0.35rem; vertical-align: middle; margin: 0.1rem 0;"
    };
    ...
    rsx! {
        div {
            ...
            if !node.children.is_empty() {
                if is_multiline {
                    div { style: "display: flex; flex-direction: column; gap: 0.25rem; padding-left: 1rem; border-left: 2px solid rgb(124 192 216 / 0.25); margin-left: 0.5rem;",
...
fn render_table_node(
    ...
    rsx! {
        div {
            style: "display: flex; flex-direction: column; border: 1px solid {border_color}; border-radius: var(--radius-md); background: rgb(0 0 0 / 0.2); overflow-x: auto; max-width: 100%; margin: 0.3rem 0; box-shadow: var(--shadow-sm);",
            onclick: move |evt: MouseEvent| {
                evt.stop_propagation();
                let mut sel = selected_node_id;
                sel.set(Some(id_for_click.clone()));
            ...
            // Table Header Row
            div { style: "display: flex; background: rgb(255 255 255 / 0.05); border-bottom: 1px solid var(--border); padding: 0.35rem 0.6rem; font-size: 0.8rem; font-weight: 600; color: var(--text-secondary);",
                div { style: "width: 2.2rem; min-width: 2.2rem; color: var(--text-secondary); text-align: center;",
                    "#"
                }
                for (i, header) in headers.iter().enumerate() {
                    div {
                        key: "th-{header}",
                        style: "width: {col_widths.get(i).copied().unwrap_or(80.0)}px; min-width: {col_widths.get(i).copied().unwrap_or(80.0)}px; padding: 0 0.4rem; text-align: left;",
                        "{header}"
                    }
                }
            }
                    ...
                    div {
                        ...
                        for (col_idx, cell) in row.children.iter().enumerate() {
                            div {
                                ...
                                style: "width: {col_widths.get(col_idx).copied().unwrap_or(80.0)}px; min-width: {col_widths.get(col_idx).copied().unwrap_or(80.0)}px; padding: 0 0.4rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;",
...
fn node_badge_style(kind: &NodeKind, is_selected: bool, is_hovered: bool) -> String {
    let (bg, text_color, border) = match kind {
        ...
        NodeKind::Keyword => (
            "rgb(192 132 252 / 0.15)",
            "#c084fc",
            "1px solid rgb(192 132 252 / 0.35)",
        ),
        NodeKind::Operator => (
            "rgb(251 191 36 / 0.15)",
            "#fbbf24",
            "1px solid rgb(251 191 36 / 0.35)",
        ),
        NodeKind::Delimiter => ("transparent", "#94a3b8", "none"),
        NodeKind::Group => (
            "rgb(124 192 216 / 0.08)",
            "#7cc0d8",
            "1px solid rgb(124 192 216 / 0.25)",
        ),
        NodeKind::Block => (
            "rgb(148 163 184 / 0.12)",
            "#cbd5e1",
            "1px solid rgb(148 163 184 / 0.3)",
        ),
        NodeKind::Table => (
            "rgb(167 139 250 / 0.12)",
            "#c4b5fd",
            "1px solid rgb(167 139 250 / 0.3)",
        ),
    };
