                    ...
                    for row in rows {
                        {
                            ...
                            let name = crate::app_state::account_display_name(&account_name_map, &row.account_id);
                            let time_str = row.latest_time.format("%Y-%m-%d %H:%M:%S").to_string();
                            let ev_count = row.event_count;

                            let activity_label = format!("{} {}", context.language.label("Latest activity:", "最新のアクティビティ:", "Lasta aktiveco:"), time_str);
