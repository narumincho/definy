...
pub fn add_record_item(root_expression: &mut definy_event::event::Expression, path: &[PathStep]) {
    if let Some(definy_event::event::Expression::TypeLiteral(record_expr)) =
        get_mut_expression_at_path(root_expression, path)
    {
        record_expr
...
pub fn add_list_item(root_expression: &mut definy_event::event::Expression, path: &[PathStep]) {
